        ...
        if GetEntityType(entity) == 3 then
            for _, blacklistedProps in pairs(Config.BlacklistProps) do
                if GetEntityModel(entity) == GetHashKey(blacklistedProps) then
                    ...
                    TriggerClientEvent('jonny-anticheat:antiProp', -1)
                    CancelEvent()
                    return
                end
            end
        elseif GetEntityType(entity) == 2 then
            for _, blacklistedVeh in pairs(Config.BlacklistVehicles) do
                if GetEntityModel(entity) == GetHashKey(blacklistedVeh) then
                    ...
                    TriggerClientEvent('jonny-anticheat:AntiVehicle', -1)
                    CancelEvent()
                    return
                end
            end
        elseif GetEntityType(entity) == 1 then
            for _, blacklistedPed in pairs(Config.BlacklistPeds) do
                if GetEntityModel(entity) == GetHashKey(blacklistedPed) then
                    ...
                    TriggerClientEvent('jonny-anticheat:antiPed', -1)
                    CancelEvent()
                    return
                end
            end
...
RegisterServerEvent('jonny-anticheat:dropplayer')
AddEventHandler('jonny-anticheat:dropplayer', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    xPlayer.kick(Config.KickMessage)
end)

RegisterServerEvent('jonny-anticheat:dclog')
AddEventHandler('jonny-anticheat:dclog', function(text)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    dclog(xPlayer, text)
end)
...
function dclog(xPlayer, text)
	...
	local data = {
        ...
        ['embeds'] = {{
          ['author'] = {
            ['name'] = 'jonny_anticheat',
            ['icon_url'] = 'https://cdn.discordapp.com/attachments/785071461516836874/802859048495349770/G3VEZE.png'
          },
          ['footer'] = {
              ['text'] = 'JONNY ANTICHEAT'
          },
          ['color'] = 12914,
          ['timestamp'] = os.date('!%Y-%m-%dT%H:%M:%SZ')
        }}
